			...
			for(int i = 0;i<=n;i++) {
				...
				if(a_bcs.get(0)==b_bcs.get(0)) {
					...
					if(a_bcs.size()==1&&b_bcs.size()==1) {
						temp=a_bcs.get(0).performance;
						continue;
					}
					if(b_bcs.size()==1) {
						// a_bcs는 그럼 사이즈가 1이상임
						temp = a_bcs.get(0).performance;
						temp=Math.max(temp, a_bcs.get(1).performance+b_bcs.get(0).performance);
						sum+=temp;
						continue;
					}
					if(a_bcs.size()==1) {
						// b_bcs는 그럼 사이즈가 1이상임
						temp = b_bcs.get(0).performance;
						temp=Math.max(temp, b_bcs.get(1).performance+a_bcs.get(0).performance);
						sum+=temp;
						continue;
					}
					// 둘다 사이즈가 1이 아닐 때
